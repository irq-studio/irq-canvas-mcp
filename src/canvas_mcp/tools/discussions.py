...
def register_discussion_tools(mcp: FastMCP):
    ...
    @mcp.tool(name="canvas_list_discussion_topics")
    @validate_params
    ...
    @mcp.tool(name="canvas_get_topic_details")
    @validate_params
    ...
    @mcp.tool(name="canvas_list_discussion_entries")
    @validate_params
    async def list_discussion_entries(course_identifier: Union[str, int],
        ...
        try:
            ...
            return f"Error: Failed to anonymize discussion entries: {str(e)}"
    ...
    @mcp.tool(name="canvas_get_entry_details")
    @validate_params
    ...
    @mcp.tool(name="canvas_get_discussion_replies")
    @validate_params
    ...
    @mcp.tool(name="canvas_post_discussion_entry")
    @validate_params
    ...
    @mcp.tool(name="canvas_reply_to_entry")
    @validate_params
    ...
    @mcp.tool(name="canvas_create_discussion")
    @validate_params
    ...
    @mcp.tool(name="canvas_list_announcements")
    ...
    @mcp.tool(name="canvas_create_announcement")
    @validate_params
    ...
    @mcp.tool(name="canvas_delete_announcement")
    @validate_params
    async def delete_announcement(course_identifier: Union[str, int],
                                announcement_id: Union[str, int]) -> str:
        """Delete a specific announcement from a course.
        
        Args:
            course_identifier: The Canvas course code (e.g., badm_554_120251_246794) or ID
            announcement_id: The Canvas announcement ID (same as discussion topic ID)
        """
        course_id = await get_course_id(course_identifier)
        
        # First get the announcement details before deleting
        announcement_response = await make_canvas_request(
            "get", f"/courses/{course_id}/discussion_topics/{announcement_id}"
        )
        
        announcement_title = "Unknown announcement"
        posted_at = "Unknown date"
        if "error" not in announcement_response:
            announcement_title = announcement_response.get("title", "Unknown announcement")
            posted_at = format_date(announcement_response.get("posted_at"))
            
            # Verify it's actually an announcement
            if not announcement_response.get("is_announcement", False):
                return f"Error: ID {announcement_id} is not an announcement (it's a regular discussion topic)"
        
        # Delete the announcement (announcements are deleted as discussion topics)
        response = await make_canvas_request(
            "delete", f"/courses/{course_id}/discussion_topics/{announcement_id}"
        )
        
        if "error" in response:
            return f"Error deleting announcement: {response['error']}"
        
        course_display = await get_course_code(course_id) or course_identifier
        
        result = f"Successfully deleted announcement from Course {course_display}:\n\n"
        result += f"Announcement: {announcement_title}\n"
        result += f"Announcement ID: {announcement_id}\n"
        result += f"Originally posted: {posted_at}\n"
        
        return result

    @mcp.tool(name="canvas_delete_discussion")
    @validate_params
    async def delete_discussion_topic(course_identifier: Union[str, int],
                                    topic_id: Union[str, int]) -> str:
        """Delete a specific discussion topic from a course.
        ...
        # First get the topic details before deleting
        topic_response = await make_canvas_request(
            "get", f"/courses/{course_id}/discussion_topics/{topic_id}"
        )
        ...
        if "error" not in topic_response:
            topic_title = topic_response.get("title", "Unknown topic")
            posted_at = format_date(topic_response.get("posted_at"))
            is_announcement = topic_response.get("is_announcement", False)
            topic_type = "Announcement" if is_announcement else "Discussion"
        
        # Delete the discussion topic
        response = await make_canvas_request(
            "delete", f"/courses/{course_id}/discussion_topics/{topic_id}"
        )
        
        if "error" in response:
            return f"Error deleting {topic_type.lower()}: {response['error']}"
        
        course_display = await get_course_code(course_id) or course_identifier
        
        result = f"Successfully deleted {topic_type.lower()} from Course {course_display}:\n\n"
        result += f"{topic_type}: {topic_title}\n"
        result += f"Topic ID: {topic_id}\n"
        result += f"Originally posted: {posted_at}\n"
        
        return result

    @mcp.tool(name="canvas_bulk_del_announcements")
    @validate_params
    async def bulk_delete_announcements(course_identifier: Union[str, int],
                                      announcement_ids: Optional[list] = None,
                                      delete_all_announcements: bool = False,
                                      older_than_days: Optional[int] = None) -> str:
        """Delete multiple announcements from a course at once.
        
        Args:
            course_identifier: The Canvas course code (e.g., badm_554_120251_246794) or ID
            announcement_ids: List of announcement IDs to delete (e.g., [123, 456, 789])
            delete_all_announcements: If True, delete ALL announcements in the course (use with caution!)
            older_than_days: Only delete announcements older than this many days
        """
        course_id = await get_course_id(course_identifier)
        
        # Safety check for delete_all_announcements
        if delete_all_announcements and not announcement_ids and older_than_days is None:
            return ("⚠️ DANGEROUS OPERATION: To delete ALL announcements, you must confirm by setting "
                   "delete_all_announcements=true AND either providing specific announcement_ids or setting older_than_days. "
                   "This is a safety measure to prevent accidental deletion of all announcements.")
        
        announcements_to_delete = []
        
        if delete_all_announcements or older_than_days is not None:
            ...
            all_announcements = await fetch_all_paginated_results(
                f"/courses/{course_id}/discussion_topics", params
            )
            
            if isinstance(all_announcements, dict) and "error" in all_announcements:
                return f"Error fetching announcements: {all_announcements['error']}"
                    ...
                    if posted_at:
                        try:
                            ...
                            if posted_date < cutoff_date:
                                announcements_to_delete.append(announcement)
                        except (ValueError, AttributeError):
                            continue
            else:
                announcements_to_delete = all_announcements
            
            if not announcements_to_delete:
                filter_desc = f"older than {older_than_days} days" if older_than_days else "to delete"
                return f"No announcements found {filter_desc} in course {course_identifier}."
        
        elif announcement_ids:
            # Convert to list of integers if needed
            try:
                announcement_ids = [int(aid) for aid in announcement_ids]
            except (ValueError, TypeError):
                return "Error: announcement_ids must be a list of integers."
            
            # Get details for specific announcements
            for aid in announcement_ids:
                ...
                if "error" not in announcement_response:
                    ...
                    if announcement_response.get("is_announcement", False):
                        announcements_to_delete.append(announcement_response)
        else:
            return "No deletion criteria specified. Use announcement_ids, delete_all_announcements=true with older_than_days, or delete_all_announcements=true with announcement_ids."
        
        if not announcements_to_delete:
            return "No announcements found to delete."
        ...
        for announcement in announcements_to_delete:
            announcement_id = announcement.get("id")
            announcement_title = announcement.get("title", "Unknown announcement")
            posted_at = format_date(announcement.get("posted_at"))
            ...
            if "error" in response:
                errors.append(f"Announcement '{announcement_title}' (ID: {announcement_id}): {response['error']}")
            else:
                results.append(f"✅ {announcement_title} (Posted: {posted_at}, ID: {announcement_id})")
        ...
        if results:
            result += f"Successfully deleted {len(results)} announcements:\n"
            for success in results:
                result += f"{success}\n"
            result += "\n"
        
        if errors:
            result += f"Failed to delete {len(errors)} announcements:\n"
            for error in errors:
                result += f"❌ {error}\n"
        
        if not results and not errors:
            result += "No announcements were processed.\n"
